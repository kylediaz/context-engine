...
export async function processNangoSyncWebhook(payload: NangoSyncWebhookBody) {
  ...
  const { connectionId, providerConfigKey, model, modifiedAfter } =
    successPayload;
  ...
  for (const record of records) {
    ...
    const unifiedDoc = transformToUnifiedDocument(
      model,
      record,
      connectionId,
      providerConfigKey,
...
async function getNangoRecords(
  connectionId: string,
...
async function deleteDocumentsFromChroma(
  documentKeys: string[],
...
function transformToUnifiedDocument(
  model: string,
  record: any,
  connectionId: string,
  providerConfigKey: string,
