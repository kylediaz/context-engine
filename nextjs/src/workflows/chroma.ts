...
export const getUserCredentials = async (
  ...
  const credentials = await db
    ...
    .innerJoin(
      userChromaCredentials,
      eq(userConnections.userId, userChromaCredentials.userId),
    )
    .limit(1)
...
async function unifiedDocumentsToChromaChunks(
  documents: UnifiedDocument[],
): Promise<ChromaChunks[]> {
  "use step";
    ...
    for (const document of batch) {
      ...
      if (content.length <= chunkSize) {
        ...
        contentChunks.forEach((chunk, chunkIndex) => {
          batchIds.push(
            `${document.source}::${document.document_key}::${chunkIndex}`,
          );
...
export function batchChromaChunks(
  chunks: ChromaChunks,
...
export async function upsertChromaChunks(
  chunks: ChromaChunks,
  credentials: ChromaCredentials,
) {
  "use step";
...
export async function deleteChromaChunks(
  documents: UnifiedDocument[],
  credentials: ChromaCredentials,
) {
  "use step";
...
export function createWhereClauses(
  documents: UnifiedDocument[],
  maxPredicates: number = 8,
): Where[] {
  let wheres: Where[] = documents.map(
    (document) =>
      ({
        $and: [
          {
